...
{
    $TargetFilePath=-join($env:SystemRoot,"\System32\OrBitScreenSaver.scr")
    $LocalMESUpdateFilesPath="e:\MESPREP"
    $LocalSCRFilePath= ($LocalMESUpdateFilesPath + "\updates\ScreenSavers\OrBitScreenSaver.scr")
    $RemoteMESUpdateFilesPath= "\\172.20.0.103\sop-file7\MESPREP\Updates"
    $RemoteSCRFilePath=-join($RemoteMESUpdateFilesPath,"\updates\ScreenSavers\OrBitScreenSaver.scr")




     if (Test-Path $RemoteSCRFilePath)
     {
            ...
            Copy-Item -Path $RemoteMESPrepFilesPath  -Recurse -Exclude "ClientConfig" -force -Destination $LocalMESPrepFilesPath -ErrorAction SilentlyContinue
            Copy-Item -Path $LocalSCRFilePath   -force -Destination $TargetFilePath -ErrorAction SilentlyContinue
