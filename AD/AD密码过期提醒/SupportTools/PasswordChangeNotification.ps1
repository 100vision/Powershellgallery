...
###################################################################################################################
# Time / Date Info
$SenderHostname =$env:COMPUTERNAME
$start = [datetime]::Now
...
$users = get-aduser -filter {(Enabled -eq $true) -and (PasswordNeverExpires -eq $false)} -searchBase "ou=05_资讯部,dc=solex-server,dc=com" -properties Name, PasswordNeverExpires, PasswordExpired, PasswordLastSet, EmailAddress | where { $_.passwordexpired -eq $false }
...
foreach ($user in $notifyUsers)
{
    ...
    # Subject Setting
    $subject="密码过期提醒：您的AD密码将"+$messagedays+"天后到期"
    # Email Body Set Here, Note You can use HTML, including Images.
    # examples here https://youtu.be/iwvQ5tPqgW0 
    $body ="
    <font face=""verdana"">
    您好 $name,
    <p> 您的密码将于 $messageDays 天后过期，请及时更改<br>
    更改方法：同时按下 CTRL+ALT+Delete 然后选择 更改密码 <br>
    <p> 如有疑问，请联系系统课支持人员 it-ops@solex.com <br>
    <p>谢谢, <br> 
    </P>
     <p> Hello $name,<br>
        <p> Your password will expire in $messageDays days，please change it asap<br>
    To change password, press CTRL ALT Delete and choose Change Password<br>
    <p>  should you have questions, feel free to contact IT support team at it-ops@solex.com <br>
    <p>Thank you, <br> 
        <p>Generated from $SenderHostname<br> 
    </P>
    </font>"
